...
`calcul-numerique.js` fait. **Prochaine étape : `algorithmique-cinquieme.js`** (dernier chapitre 5e — pas de figure requise)
...
ce fichier). `symetrie-centrale-parallelogrammes.js` commit `07daa4a` (4 figures : 2 neuves
— symétrie centrale, parallèles+sécante — et 2 réutilisant
buildParallelogrammeFigure déjà présent dans le fichier). `triangles.js` commit `ff967d6`
(4 figures réutilisant buildTriangleFigure déjà présent dans le fichier).
La partie géométrie de la 5e est terminée. `statistiques-probabilites.js` commit `4b159ba`. `proportionnalite-cinquieme.js` commit `1d42250`. `fonctions.js` commit `46829b1`. Prochaine étape :
`algorithmique-cinquieme.js` (dernier chapitre 5e — pas de figure requise,
c'est un chapitre Python/pseudocode).
