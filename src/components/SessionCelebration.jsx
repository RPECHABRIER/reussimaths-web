...
import { sessionEvidence } from "../lib/pedagogicalReliability";
import { trackProductEvent } from "../lib/productAnalytics";

function progressMessage(ratio) {
  if (ratio >= 0.8) return "Tu as répondu correctement dans cette série. Un nouvel essai lors d’une autre séance aidera à consolider ce travail.";
  if (ratio >= 0.5) return "Tu as commencé à construire la méthode. Une courte révision permettra de la rendre plus sûre.";
  return "Tu as identifié ce qu’il faut retravailler. Tu peux reprendre ce point à ton rythme.";
}
...
export default function SessionCelebration({ chapterTitle, correct, total, skillStats = {}, discoverySignup, levelId, onContinue }) {
  ...
  const { autonomous, reinforce, recovered } = sessionEvidence(skillStats);
  const nextPriority = reinforce[0] ?? null;
  return (
    <div className="w-full max-w-lg rounded-[2rem] p-5 text-center sm:p-7" style={{background:colors.card,boxShadow:shadow.floating}}>
      ...
      <h1 className="mt-1 text-2xl font-black" style={{fontFamily:fonts.display,color:colors.ink}}>{discoverySignup ? "Ta première série est terminée" : "Ta séance est terminée"}</h1>
      ...
      <div className="mt-4 rounded-2xl p-4 text-left" style={{background:colors.bg}}>
        <div className="mb-3 flex items-center justify-between"><p className="text-xs font-black" style={{color:colors.ink}}>Ton chemin de maîtrise</p><span className="flex items-center gap-1 text-[10px] font-bold" style={{color:colors.gold}}><CalendarClock size={12}/> à revoir lors d’une autre séance</span></div>
      ...
      {skillRows.length > 0 && <div className="mt-4 grid gap-2 text-left text-xs">
        <div className="rounded-2xl p-3" style={{background:`${colors.green}0d`}}><p className="font-black" style={{color:colors.green}}>Réussi sans aide aujourd’hui</p><p className="mt-1" style={{color:colors.slate}}>{autonomous.length ? autonomous.join(" · ") : "Pas encore de réussite autonome confirmée."}</p></div>
        <div className="rounded-2xl p-3" style={{background:`${colors.gold}0d`}}><p className="font-black" style={{color:colors.ink}}>À renforcer</p><p className="mt-1" style={{color:colors.slate}}>{reinforce.length ? reinforce.join(" · ") : "Aucune erreur ni aide enregistrée dans cette courte série. Un rappel ultérieur reste utile."}</p></div>
      ...
      {recovered.length > 0 && <p className="mt-3 text-sm">Après une erreur, tu as réussi sans aide une question analogue : {recovered.join(" · ")}. Cette réussite reste à consolider.</p>}
      ...
      {discoverySignup ? <>
        ...
        <Link to="/compte" onClick={() => trackProductEvent("account_cta_clicked", { source: "trial_completed", levelId })} className="mt-4 flex w-full items-center justify-center gap-2 rounded-full py-3.5 text-sm font-black" style={{background:colors.ink,color:colors.card}}>Continuer mon entraînement <ArrowRight size={15}/></Link>
        <p className="mt-2 text-[11px]" style={{color:colors.slate}}>Crée gratuitement ton espace pour sauvegarder tes progrès.</p>
