...
`calcul-numerique.js` fait. **Prochaine étape : `triangles.js`** (géométrie — figures requises)
...
buildParallelogrammeFigure déjà présent dans le fichier). Prochaine étape :
`triangles.js` (géométrie, figures requises).
