...
function RobotGame({ onBack }) {
  ...
  const preview = useMemo(() => level ? runCommands(level, commands) : null, [level, commands]);
  const restart = () => { setLevelIndex(0); setCommands([]); setFeedback(null); };
  ...
  const execute = () => { const result = runCommands(level, commands); const success = !result.outside && result.row === level.goal[0] && result.column === level.goal[1]; setFeedback(success ? "Colis livré !" : result.outside ? "Le robot est sorti du quadrillage." : "Le robot n’est pas encore sur le colis."); if (success) window.setTimeout(() => { setLevelIndex((value) => value + 1); setCommands([]); setFeedback(null); }, 950); };
  return <GameShell title="Le robot livreur" accent="#00897b" onBack={onBack}><p className="mt-2 text-sm" style={{ color: colors.slate }}>Programme le robot : avance ou pivote, puis lance ton trajet.</p><div className="mt-5 flex justify-between text-xs font-bold" style={{ color: colors.slate }}><span>Livraison {levelIndex + 1} / 5</span><span>{commands.length} / 15 instructions</span></div><div className="mx-auto mt-5 grid max-w-sm grid-cols-5 gap-1 rounded-2xl p-2" style={{ backgroundColor: `${colors.ink}0d` }}>{Array.from({ length: 25 }, (_, index) => { const row = Math.floor(index / 5), column = index % 5, robot = row === preview.row && column === preview.column && !preview.outside, goal = row === level.goal[0] && column === level.goal[1]; return <div key={index} className="flex aspect-square items-center justify-center rounded-lg text-xl" style={{ backgroundColor: goal ? `${colors.gold}38` : colors.card, border: `1px solid ${goal ? colors.gold : colors.hairline}` }}>{robot ? <span aria-label="robot">🤖<small>{["↑", "→", "↓", "←"][preview.direction]}</small></span> : goal ? <span aria-label="colis">📦</span> : ""}</div>; })}</div><div className="mt-5 grid grid-cols-3 gap-2">{["left", "forward", "right"].map((command) => <button key={command} onClick={() => commands.length < 15 && setCommands((current) => [...current, command])} className="rounded-2xl py-3 text-sm font-black" style={{ backgroundColor: `${colors.green}16`, color: colors.ink }}>{COMMAND_LABELS[command]}</button>)}</div><div className="mt-3 flex gap-2"><button onClick={() => setCommands((current) => current.slice(0, -1))} disabled={!commands.length} className="flex flex-1 items-center justify-center gap-1 rounded-full py-2 text-xs font-bold" style={{ border: `1px solid ${colors.hairline}`, color: colors.slate }}><Undo2 size={14}/> Annuler</button><button onClick={() => setCommands([])} disabled={!commands.length} className="flex flex-1 items-center justify-center gap-1 rounded-full py-2 text-xs font-bold" style={{ border: `1px solid ${colors.hairline}`, color: colors.slate }}><Delete size={14}/> Effacer</button></div><p className="mt-4 min-h-12 rounded-xl p-2 text-center text-xs" style={{ backgroundColor: colors.bg, color: colors.slate }}>{commands.length ? commands.map((command) => COMMAND_LABELS[command]).join(" → ") : "Ajoute tes instructions ici."}</p><button onClick={execute} disabled={!commands.length} className="mt-3 flex w-full items-center justify-center gap-2 rounded-full py-3 font-black" style={{ backgroundColor: commands.length ? colors.gold : colors.hairline, color: colors.ink }}><Play size={16}/> Lancer le robot</button><p aria-live="polite" className="mt-3 min-h-6 text-center text-sm font-black" style={{ color: feedback?.includes("livré") ? colors.green : colors.red }}>{feedback}</p></GameShell>;
}
