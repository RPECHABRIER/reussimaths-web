...
export default function FeedbackVisual({ family, exercise }) {
  ...
  if (family.startsWith("geometry") || family === "pythagoras") {
    ...
    if (family === "geometry_thales") {
      return <div className="mt-4 rounded-xl bg-white p-3" style={{border:`1px solid ${colors.gold}35`}}><p className="text-xs font-bold" style={{color:colors.ink}}>Conserver l’ordre des côtés correspondants</p><svg viewBox="0 0 270 150" className="mt-2 w-full" role="img" aria-label="Configuration animée du théorème de Thalès"><path d="M35 125 L135 22 L235 125 Z" fill={`${colors.gold}08`} stroke={colors.ink} strokeWidth="3"/><line x1="78" y1="81" x2="192" y2="81" stroke={colors.green} strokeWidth="4"/><line x1="35" y1="125" x2="235" y2="125" stroke={colors.gold} strokeWidth="4"/><path d="M78 81 L35 125 M192 81 L235 125" stroke={colors.gold} strokeWidth="5" strokeDasharray="8 6"><animate attributeName="stroke-dashoffset" values="28;0" dur="1.5s" repeatCount="indefinite"/></path><text x="126" y="17" fontSize="11" fill={colors.ink}>sommet commun</text><text x="112" y="76" fontSize="11" fill={colors.ink}>parallèle</text><text x="102" y="143" fontSize="11" fill={colors.ink}>côté correspondant</text></svg><p className="text-[11px] text-center" style={{color:colors.slate}}>On lit les deux triangles depuis le sommet commun et on écrit les longueurs dans le même ordre. Ici, la longueur cherchée vaut {exercise?.answerDisplay ?? exercise?.answer ?? "?"}.</p></div>;
    }
