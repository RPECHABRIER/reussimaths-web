    ...
    <div className="min-h-screen px-4 py-5 sm:px-8 sm:py-8" style={{ background: colors.bg, fontFamily: fonts.body, color: colors.ink }}>
      ...
      <main className="mx-auto max-w-4xl">
        ...
        <header className="mt-6 rounded-[2rem] px-6 py-9 sm:px-10" style={{ background: colors.ink, color: colors.card, boxShadow: shadow.raised }}>
          ...
          <h1 className="mt-3 text-3xl font-black leading-tight sm:text-5xl" style={{ fontFamily: fonts.display }}>{page.title}</h1>
        ...
        <section className="mt-10" aria-labelledby="cours">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            {page.rules.map((rule) => <article key={rule.title} className="rounded-3xl bg-white p-6" style={{ boxShadow: shadow.soft }}><h3 className="font-black">{rule.title}</h3><p className="mt-2 text-sm leading-relaxed" style={{ color: colors.slate }}>{rule.text}</p></article>)}
          </div>
        </section>
        ...
        <section className="mt-10" aria-labelledby="exercices">
          ...
          <div className="mt-4 space-y-4">
            {page.exercises.map((exercise, index) => <article key={exercise.question} className="rounded-3xl bg-white p-5 sm:p-6" style={{ boxShadow: shadow.soft }}><h3 className="font-bold">Exercice {index + 1}</h3><p className="mt-2">{exercise.question}</p><details className="mt-4 rounded-2xl p-4" style={{ background: `${colors.green}10` }}><summary className="cursor-pointer font-bold" style={{ color: colors.green }}>Voir la correction</summary><p className="mt-2 text-sm leading-relaxed">{exercise.answer}</p></details></article>)}
          </div>
        </section>
        ...
        <aside className="mt-10 rounded-[2rem] p-6 text-center sm:p-9" style={{ background: `${colors.gold}18`, border: `1px solid ${colors.gold}45` }}>
          <BookOpenCheck className="mx-auto" color={colors.gold}/><h2 className="mt-3 text-2xl font-black">Passe du cours à la maîtrise</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-relaxed" style={{ color: colors.slate }}>Continue avec des exercices générés, une correction détaillée et une progression visible dans RéussiMaths.</p><Link to={`/chapitre/${page.chapterId}`} className="mt-5 inline-flex items-center gap-2 rounded-full px-6 py-3 font-black" style={{ background: colors.ink, color: colors.card }}>Commencer l’entraînement <ArrowRight size={17}/></Link>
        </aside>
