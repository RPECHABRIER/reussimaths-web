    ...
    <div className="min-h-screen w-full p-4 sm:p-8" style={{ background: colors.bg, fontFamily: fonts.body }}>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="rounded-[2rem] px-5 py-8 sm:p-10 text-center my-7 sm:my-10 relative overflow-hidden" style={{ backgroundColor: colors.ink, boxShadow: shadow.raised }}>
          ...
          <div className="relative mx-auto rounded-2xl flex items-center justify-center" style={{ width: 54, height: 54, backgroundColor: `${colors.gold}22`, border: `1px solid ${colors.gold}44` }}><Gamepad2 size={25} color={colors.gold} /></div>
          ...
          {GAMES.map((game) => {
            ...
            return (
              <Link key={game.id} to={`/jeux/${game.id}`}>
                <div
                  className="rounded-3xl p-5 sm:p-6 flex flex-col h-full transition-transform hover:-translate-y-1 active:scale-[0.98]"
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <span className="text-[0.65rem] font-bold px-2.5 py-1 rounded-full" style={{ backgroundColor: `${game.accent}12`, color: game.accent }}>{game.duration}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between gap-3 mt-5 pt-4" style={{ borderTop: `1px solid ${colors.hairline}` }}><span className="text-xs font-semibold" style={{ color: colors.slate }}>{game.level}</span><span className="inline-flex items-center gap-1 text-xs font-bold" style={{ color: game.accent }}>Jouer <ArrowRight size={14} /></span></div>
                </div>
              </Link>
            );
          })}
