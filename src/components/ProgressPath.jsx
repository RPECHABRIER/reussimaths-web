...
export default function ProgressPath({ steps, activeIndex = 0, compact = false }) {
  return (
    <div className={compact ? "progress-path grid grid-cols-5 gap-1" : "progress-path flex flex-col"} aria-label="Chemin de progression">
      {steps.map((step, index) => {
        const done = !!step.done;
        const active = index === activeIndex && !done;
        const locked = !!step.locked;
        return (
          <div key={`${step.label}-${index}`} className={compact ? "min-w-0 text-center" : "relative flex gap-3 pb-5 last:pb-0"}>
