...
import { ArrowRight, Brain, CheckCircle2, Clock3, Flame, Target, Trophy, TrendingUp } from "lucide-react";
...
export default function DailyMentalMath() {
  ...
  if (phase === "intro") return <Shell wide>
    <div className="relative mx-auto w-fit"><Mascot size={86} motion="float"/><span className="absolute -bottom-1 -right-2 flex h-9 w-9 items-center justify-center rounded-full bg-white shadow"><Brain size={18} color={colors.green}/></span></div>
    ...
    <div className="mt-5 grid grid-cols-10 gap-1" aria-label="Une série de dix questions">{Array.from({length:10},(_,i)=><span key={i} className="h-2 rounded-full" style={{background:i<goal?colors.green:`${colors.ink}12`}}/>)}</div>
    <button onClick={start} className="mt-4 w-full rounded-full py-3.5 font-black" style={{backgroundColor:colors.gold,color:colors.ink}}>Lancer mes 10 questions <ArrowRight size={16} className="inline ml-1"/></button>
    <Link to={`/niveau/${levelId}`} className="mt-4 inline-block text-xs font-bold" style={{color:colors.slate}}>Retour au niveau</Link>
  </Shell>;

  if (phase === "result") return <Shell><div className="relative mx-auto w-fit"><Mascot size={82} motion="celebrate"/><Trophy size={28} className="absolute -bottom-1 -right-2" color={score>=goal?colors.green:colors.gold}/></div><h1 className="mt-3 text-3xl font-black" style={{color:colors.ink}}>{score} / 10</h1><div className="mx-auto mt-3 grid max-w-xs grid-cols-10 gap-1">{Array.from({length:10},(_,i)=><span key={i} className="h-2 rounded-full" style={{background:i<score?colors.green:`${colors.ink}12`}}/>)}</div><p className="mt-3 text-sm font-bold" style={{color:score>=goal?colors.green:colors.slate}}>{score>=goal?`Objectif de ${goal}/10 atteint !`:`Encore ${goal-score} point${goal-score>1?"s":""} pour atteindre ton objectif.`}</p><p className="mt-2 text-xs" style={{color:colors.slate}}>Le meilleur score de chaque journée est conservé. Rejouer permet de battre ton record sans effacer un meilleur résultat.</p><button onClick={start} className="mt-5 w-full rounded-full py-3 font-black" style={{backgroundColor:colors.gold,color:colors.ink}}>Rejouer la série</button><Link to={`/niveau/${levelId}`} className="mt-4 inline-block text-xs font-bold" style={{color:colors.slate}}>Continuer mon parcours</Link></Shell>;

  return <div className="min-h-screen flex flex-col p-4 sm:p-8" style={{background:colors.bg,fontFamily:fonts.body}}><div className="mx-auto w-full max-w-2xl"><div className="flex items-center justify-between"><CalculationModeBadge exercise={exercise} large/><div className="mental-timer flex h-14 w-14 items-center justify-center rounded-full text-sm font-black" style={{color:timeLeft<=5?colors.red:colors.ink,"--timer-progress":`${timeLeft/QUESTION_SECONDS*360}deg`,"--timer-color":timeLeft<=5?colors.red:colors.green}}>{timeLeft}s</div></div><div className="mt-4 grid grid-cols-10 gap-1" aria-label={`Question ${index+1} sur 10`}>{Array.from({length:10},(_,i)=><span key={i} className="h-2 rounded-full" style={{background:i<index?colors.green:i===index?colors.gold:`${colors.ink}12`}}/>)}</div><p className="mt-8 text-center text-xs font-bold uppercase tracking-widest" style={{color:colors.slate}}>Question {index+1} / 10</p><div className="exercise-stage mt-4 rounded-[2rem] p-7 text-center" style={{backgroundColor:colors.card,boxShadow:shadow.raised}}><p className="text-3xl sm:text-5xl font-black" style={{fontFamily:fonts.mono,color:colors.ink}}>{exercise.prompt}</p><input autoFocus inputMode="decimal" value={input} disabled={!!feedback} onChange={(event)=>setInput(event.target.value)} onKeyDown={(event)=>event.key==="Enter"&&answer(false)} placeholder="Ta réponse" className="mt-7 w-full rounded-2xl p-4 text-center text-2xl font-black" style={{backgroundColor:colors.bg,color:colors.ink,border:`1px solid ${colors.hairline}`}}/><button disabled={!input.trim()||!!feedback} onClick={()=>answer(false)} className="mt-3 w-full rounded-full py-3.5 font-black" style={{backgroundColor:colors.ink,color:colors.bg,opacity:(!input.trim()||feedback)?0.6:1}}>Valider <ArrowRight size={17} className="inline"/></button>{feedback&&<div className="feedback-moment mt-4 grid grid-cols-[auto_1fr] items-center gap-2 rounded-2xl p-3 text-left text-sm font-bold" style={{backgroundColor:feedback.correct?`${colors.green}15`:`${colors.red}12`,color:feedback.correct?colors.green:colors.ink}}>{feedback.correct?<CheckCircle2 size={20}/>:<Flame size={20} color={colors.gold}/>}<span>{feedback.correct?"Oui, bon réflexe !":<>{feedback.timedOut?"Temps écoulé. ":"Pas encore. "}{exercise.method}</>}</span></div>}</div></div></div>;
}
