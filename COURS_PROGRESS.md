...
`calcul-numerique.js` fait. **Prochaine étape : `geometrie-espace.js`** (géométrie — figures requises)
...
buildGraduatedLineFigure et buildRepereFigure déjà présents dans le
fichier). Prochaine étape : `geometrie-espace.js` (géométrie, figures
requises).
